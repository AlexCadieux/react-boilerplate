...
console.log('babel', JSON.stringify(packageJson));
const getDeps = (deps) =>
  Object.entries(deps)
    .map((dep) => `${dep[0]}@${dep[1]}`)
    .toString()
...
exec(
  ...
  (initErr, initStdout, initStderr) => {
    if (initErr) {
      console.error(`Everything was fine, but all of a sudden:
    ${initErr}`);
      return;
    }
    ...
    https.get(
      "https://raw.githubusercontent.com/AlexCadieux/react-boilerplate/master/.gitignore",
      (res) => {
    ...
    exec(
      ...
      ${npmErr}`);
        ...
        fs.copy(path.join(__dirname, "../.babelrc"), `${process.argv[2]}/.babelrc`)
          .then(() => console.log('Successfully copied babel config'))
          .catch((err) => console.error(err));
        fs.copy(path.join(__dirname, "../.eslintrc.js"), `${process.argv[2]}/.eslintrc.js`)
          .then(() => console.log('Successfully copied eslint config'))
          .catch((err) => console.error(err));
        fs.copy(path.join(__dirname, "../.tsconfig.json"), `${process.argv[2]}/tsconfig.json`)
          .then(() => console.log('Successfully copied typescript config'))
          .catch((err) => console.error(err));
